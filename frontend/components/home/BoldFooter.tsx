    ...
    <footer className="bg-[#0F0F0F]">
      ...
      <div className="max-w-[1360px] mx-auto px-6 md:px-[60px] pt-14 pb-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 lg:gap-12">
          {/* Column 1 — Shop */}
          <div>
            <h3 className="text-[11px] uppercase tracking-[0.2em] font-bold text-[#FF3C00] mb-5">
              {t("landing.footer.shop", { defaultValue: "Shop" })}
            </h3>
            <div className="w-8 h-[2px] bg-[#FF3C00] mt-2 mb-5" />
            <ul className="space-y-3">
              {NAV_LINKS.map((link) => (
                <li key={link.key}>
                  <Link
                    href={link.href}
                    className="text-[13px] text-white/60 transition-colors hover:text-[#FF3C00] cursor-pointer"
                  >
                    {t(link.key)}
                  </Link>
                </li>
          ...
          <div>
            ...
            <div className="space-y-3 text-[13px] text-white/60">
              {store?.contactEmail && (
                <p>
                  <a href={`mailto:${store.contactEmail}`} className="hover:text-[#FF3C00] transition-colors">
                    {store.contactEmail}
                  </a>
                </p>
              )}
              {store?.contactPhone && (
                <p>
                  <a href={`tel:${store.contactPhone}`} className="hover:text-[#FF3C00] transition-colors">
                    {store.contactPhone}
                  </a>
                </p>
              )}
      ...
      <div className="border-t border-white/10">
        <div className="max-w-[1360px] mx-auto px-6 md:px-[60px] py-5 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 order-2 md:order-1">
            {socialLinks.map((link) => (
              <a
                ...
                className="text-white/40 hover:text-[#FF3C00] transition-colors cursor-pointer"
              >
          ...
          {/* Copyright */}
          <p className="text-[11px] uppercase tracking-[0.12em] text-white/40 order-1 md:order-2">
