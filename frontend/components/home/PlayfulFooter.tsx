...
export function PlayfulFooter() {
  ...
  return (
    <footer className="border-t border-[#E8E5F7]">
      {/* ── Lavender 3-column body ── */}
      <div className="bg-[#F8F7FF]">
        <div className="max-w-[1360px] mx-auto px-6 md:px-[60px] pt-14 pb-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 lg:gap-12">
            {/* Column 1 — Shop */}
            <div>
              <h3 className="text-[11px] uppercase tracking-[0.2em] font-semibold mb-5 text-[#7C3AED]">
              ...
              <ul className="space-y-3">
                {NAV_LINKS.map((link) => (
                  <li key={link.key}>
                    <Link
                      href={link.href}
                      className="text-[13px] text-[#4B4869]/60 transition-colors hover:text-[#7C3AED] cursor-pointer"
                    >
                      {t(link.key)}
                    </Link>
                  </li>
            ...
            <div>
              ...
              <div className="space-y-3 text-[13px] text-[#4B4869]/60">
                {store?.contactEmail && (
                  <p>
                    <a href={`mailto:${store.contactEmail}`} className="hover:text-[#7C3AED] transition-colors">
                      {store.contactEmail}
                    </a>
                  </p>
                )}
                {store?.contactPhone && (
                  <p>
                    <a href={`tel:${store.contactPhone}`} className="hover:text-[#7C3AED] transition-colors">
                      {store.contactPhone}
                    </a>
                  </p>
                )}
      ...
      {/* ── Inverted dark copyright bar ── */}
      <div className="bg-[#2D2A4A]">
          ...
          <div className="flex items-center gap-4 order-2 md:order-1">
            {socialLinks.map((link) => (
              <a
                ...
                className="text-white/40 hover:text-[#7C3AED] transition-colors cursor-pointer"
              >
