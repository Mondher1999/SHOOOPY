    ...
    <>
      <header className="sticky top-0 z-40 w-full bg-[#0A0A0F] border-b border-[#00FF88]/20">
        <div className="flex h-[56px] items-center px-6 lg:px-10 max-w-[1600px] mx-auto">
          ...
          <Link
            href="/"
            className="text-[16px] font-mono font-bold tracking-[0.2em] uppercase whitespace-nowrap text-[#00FF88] hover:drop-shadow-[0_0_8px_rgba(0,255,136,0.5)] transition-all duration-200 focus:outline-none mr-10"
          >
            ...
            {navItems.map((item) =>
              item.children.length > 0 ? (
                <div key={item.id} className="relative group">
                  <button className="text-[11px] font-mono tracking-[0.1em] uppercase whitespace-nowrap text-white/50 hover:text-[#00FF88] hover:drop-shadow-[0_0_6px_rgba(0,255,136,0.4)] transition-all duration-200 flex items-center gap-1 cursor-pointer">
                    {item.label}
                    <ChevronDown className="w-3 h-3" />
                  </button>
                  <div className="absolute left-0 top-full pt-1 hidden group-hover:block z-50">
                    <div className="bg-[#0A0A0F] border border-[#00FF88]/20 shadow-[0_0_12px_rgba(0,255,136,0.1)] rounded py-2 min-w-[180px]">
                      <Link href={item.href} className="block px-4 py-2 text-[11px] font-mono tracking-[0.1em] uppercase text-white/50 hover:text-[#00FF88] hover:bg-[#00FF88]/5 transition-all" {...(item.openInNewTab ? { target: "_blank", rel: "noopener noreferrer" } : {})}>
                        {item.label}
                      </Link>
                      {item.children.map((child) => (
                        <Link key={child.id} href={child.href} className="block px-4 py-2 text-[11px] font-mono tracking-[0.1em] uppercase text-white/50 hover:text-[#00FF88] hover:bg-[#00FF88]/5 transition-all" {...(child.openInNewTab ? { target: "_blank", rel: "noopener noreferrer" } : {})}>
                          {child.label}
                        </Link>
                      ))}
                    </div>
          ...
          <div className="flex items-center gap-5 ml-auto">
            ...
            <button
              ...
              className="text-white/50 hover:text-[#00FF88] hover:drop-shadow-[0_0_6px_rgba(0,255,136,0.4)] transition-all duration-200 cursor-pointer focus:outline-none"
            ...
            <Link
              href="/wishlist"
              className="relative text-white/50 hover:text-[#00FF88] hover:drop-shadow-[0_0_6px_rgba(0,255,136,0.4)] transition-all duration-200 cursor-pointer focus:outline-none"
            ...
            <button
              className="lg:hidden text-white/50 hover:text-[#00FF88] hover:drop-shadow-[0_0_6px_rgba(0,255,136,0.4)] transition-all cursor-pointer focus:outline-none ml-1"
