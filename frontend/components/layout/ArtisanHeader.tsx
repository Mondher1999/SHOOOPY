  ...
  return (
    <>
      <header className="sticky top-0 z-40 w-full">
        {/* Row 1 — Tagline bar */}
        <div className="h-[28px] bg-[#2C1810] flex items-center justify-center px-6 lg:px-10">
        ...
        {/* Row 2 — Main nav */}
        <div className="h-[56px] bg-[#FFF8F0] border-b border-[#E8DDD0]">
          <div className="flex h-full items-center px-6 lg:px-10 max-w-[1600px] mx-auto">
            ...
            <nav
              className="hidden lg:flex items-center gap-7 flex-1 border-l border-dotted border-[#E8DDD0] pl-7"
              ...
              {navItems.map((item) =>
                item.children.length > 0 ? (
                  ...
                  <Link
                    key={item.id}
                    href={item.href}
                    className="text-[11px] tracking-[0.14em] uppercase whitespace-nowrap text-[#3D2B1F]/70 hover:text-[#C67B4A] transition-colors duration-200"
            ...
            <div className="flex items-center gap-5 ml-auto border-l border-dotted border-[#E8DDD0] pl-5">
              ...
              >
                ...
                {user && wishlistCount > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 h-[14px] min-w-[14px] rounded-full text-[9px] flex items-center justify-center font-medium bg-[#C67B4A] text-white px-0.5" aria-hidden="true">
              ...
              {user ? (
                ...
                <Link
                  href="/auth/login"
                  className="hidden lg:block text-[11px] font-medium tracking-[0.12em] uppercase whitespace-nowrap text-[#3D2B1F]/70 hover:text-[#C67B4A] transition-colors duration-200"
                >
                  {t("nav.signIn")}
                </Link>
              )}
        ...
        {/* Search panel */}
        {searchOpen && (
          <div className="border-t border-[#E8DDD0] bg-[#FFF8F0] animate-in fade-in-0 slide-in-from-top-1 duration-200">
        ...
        {/* Mobile nav */}
        {mobileOpen && (
          <div className="lg:hidden border-t border-dotted border-[#E8DDD0] bg-[#FFF8F0] animate-in fade-in-0 slide-in-from-top-1 duration-200">
            <nav className="px-6 py-6 space-y-5" aria-label={t("nav.mainNav")}>
              {navItems.map((item) => (
                <div key={item.id}>
                  <Link
                    href={item.href}
                    className="block text-[12px] tracking-[0.18em] uppercase text-[#3D2B1F]/70 hover:text-[#C67B4A] transition-colors"
              ...
              {!user && (
                <Link
                  href="/auth/login"
                  className="block text-[12px] tracking-[0.18em] uppercase text-[#3D2B1F]/70 hover:text-[#C67B4A] transition-colors border-t border-dotted border-[#E8DDD0] pt-5"
                  onClick={() => setMobileOpen(false)}
                >
                  {t("nav.signIn")}
                </Link>
