    ...
    <>
      <header className="sticky top-0 z-40 w-full bg-white border-b border-[#e8e8e8]">
        <div className="flex h-[56px] items-center px-6 lg:px-10 max-w-[1600px] mx-auto">
          ...
          <Link
            href="/"
            className="font-heading text-[18px] font-normal tracking-[0.24em] uppercase whitespace-nowrap text-[#1c1c1c] hover:text-[#6b6b6b] transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded mr-10"
          >
            ...
            {navItems.map((item) =>
              item.children.length > 0 ? (
                <NavDropdown
                  ...
                  buttonClassName="text-[11px] font-medium tracking-[0.14em] uppercase whitespace-nowrap text-[#1c1c1c] hover:text-[#6b6b6b] transition-colors duration-200 flex items-center gap-1 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded"
                  menuClassName="bg-white border border-[#e8e8e8] shadow-md rounded py-2 min-w-[180px]"
                >
                  <Link href={item.href} className="block px-4 py-2 text-[11px] tracking-[0.1em] uppercase text-[#1c1c1c] hover:bg-[#f5f5f5] transition-colors focus:outline-none focus-visible:bg-[#f5f5f5]" role="menuitem" tabIndex={-1} {...(item.openInNewTab ? { target: "_blank", rel: "noopener noreferrer" } : {})}>
                    {item.label}
                  </Link>
                  {item.children.map((child) => (
                    <Link key={child.id} href={child.href} className="block px-4 py-2 text-[11px] tracking-[0.1em] uppercase text-[#1c1c1c] hover:bg-[#f5f5f5] transition-colors focus:outline-none focus-visible:bg-[#f5f5f5]" role="menuitem" tabIndex={-1} {...(child.openInNewTab ? { target: "_blank", rel: "noopener noreferrer" } : {})}>
                      {child.label}
                    </Link>
                  ))}
                </NavDropdown>
                ...
                <Link
                  key={item.id}
                  href={item.href}
                  className="text-[11px] font-medium tracking-[0.14em] uppercase whitespace-nowrap text-[#1c1c1c] hover:text-[#6b6b6b] transition-colors duration-200"
          ...
          <div className="flex items-center gap-5 ml-auto">
            ...
            {user ? (
              ...
              <Link
                href="/auth/login"
                className="hidden lg:block text-[11px] font-medium tracking-[0.12em] uppercase whitespace-nowrap text-[#1c1c1c] hover:text-[#6b6b6b] transition-colors duration-200"
              >
                {t("nav.signIn")}
              </Link>
            )}
                ...
                <div key={item.id}>
                  ...
                  {item.children.length > 0 && (
                    <div className="ml-4 mt-3 space-y-3">
                      {item.children.map((child) => (
                        <Link key={child.id} href={child.href} className="block text-[11px] tracking-[0.14em] uppercase text-[#6b6b6b] hover:text-[#1c1c1c] transition-colors" onClick={() => setMobileOpen(false)} {...(child.openInNewTab ? { target: "_blank", rel: "noopener noreferrer" } : {})}>
                          {child.label}
                        </Link>
                      ))}
                    </div>
