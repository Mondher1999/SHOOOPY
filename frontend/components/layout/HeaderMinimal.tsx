    ...
    <>
      <header className="sticky top-0 z-40 w-full bg-white border-b border-[#e8e8e8]">
        <div className="flex h-[56px] items-center px-6 lg:px-10 max-w-[1600px] mx-auto">
          ...
          <Link
            href="/"
            className="font-heading text-[18px] font-normal tracking-[0.24em] uppercase whitespace-nowrap text-[#1c1c1c] hover:text-[#6b6b6b] transition-colors duration-200 focus:outline-none mr-10"
          >
            ...
            {navItems.map((item) =>
              item.children.length > 0 ? (
                <div key={item.id} className="relative group">
                  <button className="text-[11px] font-medium tracking-[0.14em] uppercase whitespace-nowrap text-[#1c1c1c] hover:text-[#6b6b6b] transition-colors duration-200 flex items-center gap-1 cursor-pointer">
                    {item.label}
                    <ChevronDown className="w-3 h-3" />
                  </button>
                  <div className="absolute left-0 top-full pt-1 hidden group-hover:block z-50">
                    <div className="bg-white border border-[#e8e8e8] shadow-md rounded py-2 min-w-[180px]">
                      <Link href={item.href} className="block px-4 py-2 text-[11px] tracking-[0.1em] uppercase text-[#1c1c1c] hover:bg-[#f5f5f5] transition-colors" {...(item.openInNewTab ? { target: "_blank", rel: "noopener noreferrer" } : {})}>
                        {item.label}
                      </Link>
                      {item.children.map((child) => (
                        <Link key={child.id} href={child.href} className="block px-4 py-2 text-[11px] tracking-[0.1em] uppercase text-[#1c1c1c] hover:bg-[#f5f5f5] transition-colors" {...(child.openInNewTab ? { target: "_blank", rel: "noopener noreferrer" } : {})}>
                          {child.label}
                        </Link>
                      ))}
                    </div>
